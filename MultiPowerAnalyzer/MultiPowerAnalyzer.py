...
def main():
    rootPath = getPath(ROOT_PATH)

    for rP in rootPath:
         doAnalyze(rP)
    return

def doAnalyze(rootPath):
    bootPath = getPath(rootPath+BOOT_PATH)
    ...
    print ("start parsing "+rootPath.split("\\")[1])
    f = open(rootPath.split("\\")[1]+"_"+OUTPUTFILE_NAME+OUTPUTFILE_EXT,'w')
